...
@section('content')
    ...
    <form action="{{ route('admin.umkms.update', $umkm->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="nama" name="nama" value="{{ old('nama', $umkm->nama) }}" required>
            @error('nama') <div class="text-danger">{{ $message }}</div> @enderror
        </div>
        <div class="form-group">
            <label for="kategori">Kategori</label>
            <input type="text" class="form-control" id="kategori" name="kategori" value="{{ old('kategori', $umkm->kategori) }}" required>
        ...
        <div class="form-group">
            ...
            @if($umkm->gambar)
                <div class="mt-2">
                    <p>Gambar saat ini:</p>
                    <img src="{{ asset('storage/' . $umkm->gambar) }}" alt="Gambar UMKM" style="max-width: 200px; height: auto; border-radius: 8px;">
                </div>
            @endif
        </div>
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="alamat" name="alamat" value="{{ old('alamat', $umkm->alamat) }}">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="kontak_telepon" name="kontak_telepon" value="{{ old('kontak_telepon', $umkm->kontak_telepon) }}">
            @error('kontak_telepon') <div class="text-danger">{{ $message }}</div> @enderror
        </div>
        <div class="form-group">
            <label for="kontak_email">Kontak Email (Opsional)</label>
            <input type="email" class="form-control" id="kontak_email" name="kontak_email" value="{{ old('kontak_email', $umkm->kontak_email) }}">
            @error('kontak_email') <div class="text-danger">{{ $message }}</div> @enderror
        </div>
        <div class="form-group">
            <label for="harga">Harga (Opsional)</label>
            <input type="text" class="form-control" id="harga" name="harga" value="{{ old('harga', $umkm->harga) }}">
...
@section('scripts')
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            console.log("DOMContentLoaded terpicu. Menginisialisasi peta untuk Edit UMKM.");
            ...
            // LOGIKA PENTING UNTUK EDIT: Memuat koordinat UMKM yang sudah ada
            ...
            if (existingLat && existingLng) {
                ...
                if (isValidCoordinates(lat, lng)) {
                    map.setView([lat, lng], 15); // Zoom lebih dekat ke lokasi UMKM yang sudah ada
                    if (marker) {
                        map.removeLayer(marker);
                    }
                    marker = L.marker([lat, lng], { draggable: true }).addTo(map);
                    updateLocationInfo(lat, lng); // Perbarui UI info lokasi
                    marker.on('dragend', function(e) {
                        updateLocationInfo(e.target.getLatLng().lat, e.target.getLatLng().lng);
                    });
                } else {
                    console.warn("Koordinat UMKM yang ada tidak valid. Menggunakan lokasi pengguna atau default.");
                    initMapDefaultOrGeolocation();
                }
            } else {
                // Jika tidak ada koordinat UMKM yang sudah ada, coba dapatkan lokasi pengguna
                initMapDefaultOrGeolocation();
            }
            ...
            function initMapDefaultOrGeolocation() {
                if (navigator.geolocation) {
                    navigator.geolocation.getCurrentPosition(function(position) {
                        ...
                        if (!marker) { // Hanya tambahkan marker jika belum ada
                            marker = L.marker([lat, lng], { draggable: true }).addTo(map);
                            updateLocationInfo(lat, lng);
                            marker.on('dragend', function(e) {
                                updateLocationInfo(e.target.getLatLng().lat, e.target.getLatLng().lng);
